...
async def update_user(updated_user: schemas.UpdateUser, db: AsyncSession = Depends(get_db), current_user = Depends(oauth2.get_current_user)):
...
async def get_user(db: AsyncSession = Depends(get_db), current_user = Depends(oauth2.get_current_user)):
...
async def delete_user(db: AsyncSession = Depends(get_db), current_user = Depends(oauth2.get_current_user)):
